...
**You are painting one city block: block 28.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-28-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-28-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-28-layout.png` — **the plan: where the landmark goes.** The block's shape
...
- `block-28-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **540 × 752 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-28-canvas.png`. This is not a
...
- **One PNG, exactly 540 × 752 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (135 × 188 px at position x 290, y 1537 on the
...
- **The white shape in `block-28-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-28-style-reference.png`** — its brushwork, not its
...
See `block-28-context.png` — your block outlined in red dashes on the actual base
...
- **Centre it on canvas px (110, 738)**, facing its street. This is a tight corner of the block — turn the building to follow its
...
- **Paint it about 210 × 183 px** — that is 1.3× its real 24 × 21 m
...
- **Y-Not II** (bar) — around (110, 738)
- **Zipcar** (car_sharing) — around (265, 6)
- **Coffeetails** (cafe) — around (14, 630)
- **Patrician Row Condominiums** (terrace) — around (469, 89)
- **Metro Hand Car Wash & Detailing Centre** (car_wash) — around (86, 562)
...
- **Don't change the canvas shape.** 540 × 752 (0.72 : 1, portrait) — a
...
- [ ] **Y-Not II** is in the painting, near canvas px (110, 738), roughly 210 × 183 px, and obviously bigger and more interesting than the houses around it.
- [ ] Nothing in a red box on the layout plan has been left out.
- [ ] The canvas is 540 × 752 px.

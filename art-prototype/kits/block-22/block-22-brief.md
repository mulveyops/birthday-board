...
**You are painting one city block: block 22.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-22-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-22-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-22-layout.png` — **the site plan, and the most important attachment
...
- `block-22-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **580 × 1712 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-22-canvas.png`. This is not a
...
> **same ratio (0.34 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 580 × 1712 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (145 × 428 px at position x 1033, y 1074 on the
...
- **The white shape in `block-22-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-22-style-reference.png`** — its brushwork, not its
...
See `block-22-context.png` — your block outlined in red dashes on the actual base
...
- **Don't change the canvas shape.** 580 × 1712 (0.34 : 1, portrait) — a

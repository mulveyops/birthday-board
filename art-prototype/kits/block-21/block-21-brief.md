...
**You are painting one city block: block 21.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-21-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-21-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-21-layout.png` — **the plan: where the landmark goes.** The block's shape
...
- `block-21-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **540 × 1704 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-21-canvas.png`. This is not a
...
> **same ratio (0.32 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 540 × 1704 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (135 × 426 px at position x 293, y 1067 on the
...
- **The white shape in `block-21-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-21-style-reference.png`** — its brushwork, not its
...
- **East:** North Cass Street
- **South:** East Pleasant Street
- **West:** North Van Buren Street

See `block-21-context.png` — your block outlined in red dashes on the actual base
...
### 1. Angelo's Piano Lounge

- **Centre it on canvas px (105, 74)**, facing its street. The whole building must sit inside the white area of the stencil: nothing on the finished map is worse than a landmark sliced in half by a street.
- **Paint it about 174 × 167 px** — that is 1.4× its real 17 × 17 m
  footprint, and roughly 32% of the block's width. Oversized on purpose.
...
- **Cass Street Boys & Girls Club** (community_centre) — around (481, 1039)
- **Cass Street School** (school) — around (428, 851)
- **Lucky Liu's** (restaurant) — around (118, 562)
- **Mangos Cafe** (bar) — around (42, 219)
- **Studio D Day Spa** (beauty) — around (36, 641)
- **Three Holy Women Parish - Saint Rita Church** (place_of_worship) — around (462, 1406)
- **Saint Rita Square** (social_facility) — around (273, 1534)
...
- [ ] **Angelo's Piano Lounge** is in the painting, near canvas px (105, 74), roughly 174 × 167 px, and obviously bigger and more interesting than the houses around it.
- [ ] Nothing in a red box on the layout plan has been left out.
- [ ] The canvas is 540 × 1704 px.

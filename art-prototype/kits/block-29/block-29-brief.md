...
**You are painting one city block: block 29.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-29-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-29-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-29-layout.png` — **the site plan, and the most important attachment
...
- `block-29-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **536 × 752 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-29-canvas.png`. This is not a
...
> **same ratio (0.71 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 536 × 752 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (134 × 188 px at position x 467, y 1539 on the
...
- **The white shape in `block-29-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-29-style-reference.png`** — its brushwork, not its
...
See `block-29-context.png` — your block outlined in red dashes on the actual base
...
### 1. 811 East Pleasant — the party house

- **It stands in the top-left corner of the canvas**, centred near
  px (171, 53), its front facing the street. The whole building must sit inside the white area of the stencil: nothing on the finished map is worse than a landmark sliced in half by a street.
- **Paint it about 200 × 125 px** — that is 1.8× its real 16 × 10 m
  footprint, and roughly 37% of the block's width. Oversized on purpose.
...
- **Clear a halo of ~97 px (14 m) around it** — inside that halo only its own
...
- **Desmond-Farnham-Hustis House** (house) — around (442, 207)
- **Graham Row** (terrace) — around (451, 695)
...
- 3 × flowering ornamental tree
- 1 × detached garage (alley side)
- 1 × duplex
- 1 × house
- 1 × mature elm (landmark size) tree
...
- **Don't change the canvas shape.** 536 × 752 (0.71 : 1, portrait) — a
...
- [ ] **811 East Pleasant — the party house** — the cream-brick house dressed for a party — is in the top-left corner, roughly 200 × 125 px, and looks like what it is: not grander, not more civic, not a different kind of building.

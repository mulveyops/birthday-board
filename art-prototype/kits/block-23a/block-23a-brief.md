...
**You are painting HALF of one city block — the first half of block 23.**
...
- `block-23a-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-23a-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-23a-layout.png` — **the site plan, and the most important attachment
...
- `block-23a-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **636 × 1016 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-23a-canvas.png`. This is not a
...
- **One PNG, exactly 636 × 1016 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (159 × 254 px at position x 1215, y 1075 on the
...
- **The white shape in `block-23a-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-23a-style-reference.png`** — its brushwork, not its
...
See `block-23a-context.png` — your block outlined in red dashes on the actual base
...
- **Don't change the canvas shape.** 636 × 1016 (0.63 : 1, portrait) — a

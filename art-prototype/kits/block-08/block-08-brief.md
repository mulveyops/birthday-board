...
**You are painting one city block: block 8.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-08-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-08-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-08-layout.png` — **the site plan, and the most important attachment
...
- `block-08-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **616 × 372 px — LANDSCAPE, wider than it is tall**
>
> Identical in size and shape to the attached `block-08-canvas.png`. This is not a
...
> **same ratio (1.66 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 616 × 372 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (154 × 93 px at position x 653, y 788 on the
...
- **The white shape in `block-08-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-08-style-reference.png`** — its brushwork, not its
...
- **Don't change the canvas shape.** 616 × 372 (1.66 : 1, landscape) — a

...
**You are painting one city block: block 7.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-07-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-07-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-07-layout.png` — **the site plan, and the most important attachment
...
> ## ⚠ OUTPUT SIZE: **600 × 372 px — LANDSCAPE, wider than it is tall**
>
> Identical in size and shape to the attached `block-07-canvas.png`. This is not a
...
> **same ratio (1.61 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 600 × 372 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (150 × 93 px at position x 459, y 786 on the
...
- **The white shape in `block-07-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-07-style-reference.png`** — its brushwork, not its
...
- **Don't change the canvas shape.** 600 × 372 (1.61 : 1, landscape) — a

...
**You are painting HALF of one city block — the first half of block 21.**
...
- `block-21a-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-21a-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-21a-layout.png` — **the site plan, and the most important attachment
...
- `block-21a-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **540 × 852 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-21a-canvas.png`. This is not a
...
> **same ratio (0.63 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 540 × 852 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (135 × 213 px at position x 293, y 1067 on the
...
- **The white shape in `block-21a-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-21a-style-reference.png`** — its brushwork, not its
...
See `block-21a-context.png` — your block outlined in red dashes on the actual base
...
- **Angelo's Piano Lounge** (bar) — around (45, 132)
...
- **Don't change the canvas shape.** 540 × 852 (0.63 : 1, portrait) — a

...
**You are painting one city block: block 26.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-26-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-26-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-26-style-reference.png` — **the look to match.** A different block of
...
> ## ⚠ OUTPUT SIZE: **660 × 760 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-26-canvas.png`. This is not a
...
> **same ratio (0.87 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 660 × 760 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (165 × 190 px at position x 826, y 1309 on the
...
- **The white shape in `block-26-canvas.png` is what you are painting**: the real
...
**Match `block-26-style-reference.png`.** It is another block of this same map,
...
See `block-26-context.png` — your block outlined in red dashes on the actual base
...
- **Buemming House** (house) — around (181, 686)
...
- **Don't change the canvas shape.** 660 × 760 (0.87 : 1, portrait) — a

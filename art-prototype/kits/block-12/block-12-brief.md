...
**You are painting one city block: block 12.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-12-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-12-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-12-layout.png` — **the site plan, and the most important attachment
...
- `block-12-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **528 × 948 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-12-canvas.png`. This is not a
...
> **same ratio (0.56 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 528 × 948 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (132 × 237 px at position x 1485, y 795 on the
...
- **The white shape in `block-12-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-12-style-reference.png`** — its brushwork, not its
...
- **East:** North Warren Avenue
- **South:** East Brady Street
- **West:** North Arlington Place

See `block-12-context.png` — your block outlined in red dashes on the actual base
...
- **Zipcar Brady & Arlington** (car_sharing) — around (106, 608)
- **Apollo Cafe** (cafe) — around (81, 897)
- **La Masa Empanada Bar** (restaurant) — around (31, 896)
- **Fro Zone** (ice_cream) — around (232, 894)
- **Uncommon Items** (clothes) — around (179, 907)
- **Qdoba** (fast_food) — around (494, 879)
- **Jimmy John's** (fast_food) — around (427, 878)
...
- 8 × maple tree
- 4 × flowering ornamental tree
- 4 × parked car
- 3 × bungalow
- 3 × mixed-use commercial (shops below, flat above)
...
- **Don't change the canvas shape.** 528 × 948 (0.56 : 1, portrait) — a

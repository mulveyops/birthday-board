...
**You are painting one city block: block 17.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-17-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-17-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-17-layout.png` — **the site plan, and the most important attachment
...
- `block-17-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **608 × 848 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-17-canvas.png`. This is not a
...
- **One PNG, exactly 608 × 848 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (152 × 212 px at position x 1418, y 1077 on the
...
- **The white shape in `block-17-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-17-style-reference.png`** — its brushwork, not its
...
See `block-17-context.png` — your block outlined in red dashes on the actual base
...
- **Jack's American Pub** (pub) — around (333, -1)
- **DORSIA** (restaurant) — around (105, 51)
- **MKE's Smallest Bar** (bar) — around (501, 33)

## Texture guidance — paint the vibe, counts are approximate

What's really on this block (from city data):

- 9 × flowering ornamental tree
- 7 × maple tree
- 3 × bungalow
- 3 × Polish flat
- 2 × bike rack

...
**You are painting one city block: block 24.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-24-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-24-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-24-layout.png` — **the site plan, and the most important attachment
...
- `block-24-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
- **One PNG, exactly 544 × 760 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (136 × 190 px at position x 647, y 1307 on the
...
- **The white shape in `block-24-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-24-style-reference.png`** — its brushwork, not its
...
See `block-24-context.png` — your block outlined in red dashes on the actual base

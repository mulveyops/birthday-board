...
**You are painting one city block: block 5.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-05-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-05-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-05-layout.png` — **the site plan, and the most important attachment
...
- `block-05-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **624 × 384 px — LANDSCAPE, wider than it is tall**
>
> Identical in size and shape to the attached `block-05-canvas.png`. This is not a
...
> **same ratio (1.63 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 624 × 384 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (156 × 96 px at position x 655, y 648 on the
...
- **The white shape in `block-05-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-05-style-reference.png`** — its brushwork, not its
...
See `block-05-context.png` — your block outlined in red dashes on the actual base
...
- **The Bikesmiths** (bicycle) — around (224, 326)
...
- **Don't change the canvas shape.** 624 × 384 (1.63 : 1, landscape) — a

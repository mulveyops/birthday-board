...
**You are painting one city block: block 3.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-03-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-03-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-03-layout.png` — **the site plan, and the most important attachment
...
- `block-03-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **748 × 1176 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-03-canvas.png`. This is not a
...
> **same ratio (0.64 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 748 × 1176 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (187 × 294 px at position x 1046, y 454 on the
...
- **The white shape in `block-03-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-03-style-reference.png`** — its brushwork, not its
...
Bounded by:
- **North:** East Kane Place
- **East:** North Pulaski Street
- **South:** East Hamilton Street
- **West:** North Humboldt Avenue

See `block-03-context.png` — your block outlined in red dashes on the actual base
...
- **Curry Hut** (restaurant) — around (22, 17)
...
- **Don't change the canvas shape.** 748 × 1176 (0.64 : 1, portrait) — a

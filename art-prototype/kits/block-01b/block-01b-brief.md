...
**You are painting HALF of one city block — the second half of block 1.**
...
- `block-01b-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-01b-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-01b-layout.png` — **the site plan, and the most important attachment
...
- `block-01b-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **628 × 608 px — LANDSCAPE, wider than it is tall**
>
> Identical in size and shape to the attached `block-01b-canvas.png`. This is not a
...
> **same ratio (1.03 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 628 × 608 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (157 × 152 px at position x 849, y 452 on the
...
- **The white shape in `block-01b-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-01b-style-reference.png`** — its brushwork, not its
...
See `block-01b-context.png` — your block outlined in red dashes on the actual base
...
- **It stands in the top-right corner of the canvas**, centred near
  px (543, 37), its front facing the street. The whole building must sit inside the white area of the stencil: nothing on the finished map is worse than a landmark sliced in half by a street.
...
- **Don't change the canvas shape.** 628 × 608 (1.03 : 1, landscape) — a

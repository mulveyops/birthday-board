...
**You are painting one city block: block 31.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-31-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-31-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-31-layout.png` — **the site plan, and the most important attachment
...
- `block-31-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **660 × 748 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-31-canvas.png`. This is not a
...
> **same ratio (0.88 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 660 × 748 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (165 × 187 px at position x 822, y 1543 on the
...
- **The white shape in `block-31-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-31-style-reference.png`** — its brushwork, not its
...
See `block-31-context.png` — your block outlined in red dashes on the actual base
...
- **Pleasant Terrace Apartments** (social_facility) — around (340, 180)
...
- **Don't change the canvas shape.** 660 × 748 (0.88 : 1, portrait) — a

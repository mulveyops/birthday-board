...
**You are painting one city block: block 10.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-10-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-10-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-10-layout.png` — **the site plan, and the most important attachment
...
- `block-10-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **316 × 316 px — SQUARE**
>
> Identical in size and shape to the attached `block-10-canvas.png`. This is not a
...
> **same ratio (1.00 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 316 × 316 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (79 × 79 px at position x 337, y 798 on the
...
- **The white shape in `block-10-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-10-style-reference.png`** — its brushwork, not its
...
- **South:** East Pearson Street
- **West:** (no named street — board edge or alley)

See `block-10-context.png` — your block outlined in red dashes on the actual base
...
- **Don't change the canvas shape.** 316 × 316 (1.00 : 1, square) — a

...
- `block-01a-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-01a-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-01a-layout.png` — **the site plan, and the most important attachment
...
- `block-01a-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **1352 × 1064 px — LANDSCAPE, wider than it is tall**
>
> Identical in size and shape to the attached `block-01a-canvas.png`. This is not a
...
> **same ratio (1.27 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 1352 × 1064 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (338 × 266 px at position x 473, y 476 on the
...
- **The white shape in `block-01a-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-01a-style-reference.png`** — its brushwork, not its
...
Bounded by:
- **North:** North Water Street
- **East:** North Astor Street / North Humboldt Avenue
- **South:** East Land Place
- **West:** East Hamilton Street / North Marshall Street

See `block-01a-context.png` — your block outlined in red dashes on the actual base
...
- **It stands in the centre of the canvas**, centred near
  px (581, 480), its front facing the street. This is a tight corner of the block — turn the building to follow its
...
- **Paint it about 390 × 100 px** — that is 1.3× its real 43 × 11 m
  footprint, and roughly 29% of the block's width. Oversized on purpose.
...
- **The 1818 Lofts** (industrial) — around (178, 901)
- **Gallun Tannery Row** — around (196, 870)

## Texture guidance — paint the vibe, counts are approximate

What's really on this block (from city data):

- 17 × flowering ornamental tree
- 8 × honeylocust tree
- 7 × maple tree
...
- **Don't change the canvas shape.** 1352 × 1064 (1.27 : 1, landscape) — a
...
- [ ] **Red Lion Pub** — the long two-storey brick pub — is in the centre, roughly 390 × 100 px, and looks like what it is: not grander, not more civic, not a different kind of building.

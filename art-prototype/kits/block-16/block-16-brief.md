...
**You are painting one city block: block 16.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-16-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-16-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-16-layout.png` — **the site plan, and the most important attachment
...
- `block-16-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **600 × 404 px — LANDSCAPE, wider than it is tall**
>
> Identical in size and shape to the attached `block-16-canvas.png`. This is not a
...
> **same ratio (1.49 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 600 × 404 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (150 × 101 px at position x 848, y 927 on the
...
- **The white shape in `block-16-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-16-style-reference.png`** — its brushwork, not its
...
Bounded by:
- **North:** East Pearson Street
- **East:** North Humboldt Avenue
- **South:** East Brady Street
- **West:** North Astor Street

See `block-16-context.png` — your block outlined in red dashes on the actual base
...
- **Dryhootch Coffeehouse** (cafe) — around (404, 330)
- **Cucina** (kitchen) — around (248, 294)
- **Shawarma Palace** (fast_food) — around (583, 396)
- **Emperor of China** (restaurant) — around (113, 315)
- **Dryhootch** (social_facility) — around (396, 282)
- **Regano's Roman Coin** (pub) — around (5, 356)
...
- **Don't change the canvas shape.** 600 × 404 (1.49 : 1, landscape) — a

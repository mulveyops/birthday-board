...
**You are painting one city block: block 9.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-09-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-09-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-09-layout.png` — **the site plan, and the most important attachment
...
- **One PNG, exactly 604 × 372 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (151 × 93 px at position x 849, y 790 on the
...
- **The white shape in `block-09-canvas.png` is what you are painting**: the real

...
**You are painting one city block: block 20.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-20-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-20-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-20-layout.png` — **the plan: where the landmark goes.** The block's shape
...
- `block-20-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
>
> Identical in size and shape to the attached `block-20-canvas.png`. This is not a
...
> **same ratio (0.86 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 664 × 772 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (166 × 193 px at position x 829, y 1072 on the
...
- **The white shape in `block-20-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-20-style-reference.png`** — its brushwork, not its
...
- **Don't change the canvas shape.** 664 × 772 (0.86 : 1, portrait) — a

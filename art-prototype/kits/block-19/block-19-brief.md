...
**You are painting one city block: block 19.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-19-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-19-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-19-layout.png` — **the plan: where the landmark goes.** The block's shape
...
- `block-19-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **544 × 788 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-19-canvas.png`. This is not a
...
> **same ratio (0.69 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 544 × 788 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (136 × 197 px at position x 471, y 1069 on the
...
- **The white shape in `block-19-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-19-style-reference.png`** — its brushwork, not its
...
- **South:** East Kewaunee Street
- **West:** North Cass Street

See `block-19-context.png` — your block outlined in red dashes on the actual base
...
- **Sherwin-Williams** (paint) — around (82, 30)
- **Sip & Purr Cat Café** (cafe) — around (470, 2)
...
- **Don't change the canvas shape.** 544 × 788 (0.69 : 1, portrait) — a

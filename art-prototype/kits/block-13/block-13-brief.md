...
**You are painting one city block: block 13.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-13-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-13-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-13-layout.png` — **the site plan, and the most important attachment
...
> ## ⚠ OUTPUT SIZE: **868 × 948 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-13-canvas.png`. This is not a
...
> **same ratio (0.92 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 868 × 948 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (217 × 237 px at position x 1224, y 794 on the
...
- **The white shape in `block-13-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-13-style-reference.png`** — its brushwork, not its
...
- **It stands in the bottom-right corner of the canvas**, centred near
  px (827, 891), its front facing the street. This is a tight corner of the block — turn the building to follow its
...
- [ ] **The Hi Hat** — the corner bar in two adjoining buildings — is in the bottom-right corner, roughly 195 × 221 px, and looks like what it is: not grander, not more civic, not a different kind of building.

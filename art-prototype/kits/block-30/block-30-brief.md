...
**You are painting one city block: block 30.** It is bounded by real streets,
it contains real buildings, and your painting drops into the hole where that
block sits.

## What is attached

- `block-30-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-30-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-30-layout.png` — **the site plan, and the most important attachment
...
- `block-30-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **544 × 752 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-30-canvas.png`. This is not a
...
- **One PNG, exactly 544 × 752 px, painted edge to edge.** That is 4× the block's
...
- **The white shape in `block-30-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-30-style-reference.png`** — its brushwork, not its
...
- **Don't change the canvas shape.** 544 × 752 (0.72 : 1, portrait) — a

...
**You are painting HALF of one city block — the first half of block 22.**
...
> **Your bottom edge is NOT a street.** It is the cut through the middle of the
...
>
> If you put a pavement along the bottom edge, the finished block ends up with a
...
- `block-22a-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-22a-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-22a-layout.png` — **the site plan, and the most important attachment
...
- `block-22a-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
> ## ⚠ OUTPUT SIZE: **580 × 856 px — PORTRAIT, taller than it is wide**
>
> Identical in size and shape to the attached `block-22a-canvas.png`. This is not a
...
> **same ratio (0.68 : 1)** and the same orientation. Never a default 4:3 or
> 16:9 canvas, and never the other orientation.

## Deliverable

- **One PNG, exactly 580 × 856 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (145 × 214 px at position x 1033, y 1074 on the
...
- **The white shape in `block-22a-canvas.png` is what you are painting**: the real
...
- **Let the ground — and only the ground — spill about 69 px past that
...
- **The perimeter band of your painting is the sidewalk/terrace zone** (~6 m
  ...
  trees in the grass terrace strip alongside it. **Except along the bottom edge** — that one is the cut through the block, not a street, and gets no pavement at all.
...
**Paint in the manner of `block-22a-style-reference.png`** — its brushwork, not its
...
- **South:** **not a street** — this is the cut through the block, and the other half carries on from it
- **West:** North Humboldt Avenue

See `block-22a-context.png` — your block outlined in red dashes on the actual base
...
- **It stands in the top-right corner of the canvas**, centred near
  px (573, 140), its front facing the street. This is a tight corner of the block — turn the building to follow its
...
- **Paint it about 158 × 88 px** — that is 1.3× its real 18 × 10 m
...
- **Peter Sciortino's Bakery** (bakery) — around (80, 23)
...
- 7 × Polish flat
- 7 × honeylocust tree
- 4 × mature elm (landmark size) tree
- 4 × linden tree
- 2 × bike rack
- 2 × mixed-use commercial (shops below, flat above)
- 2 × duplex
- 2 × oak tree
...
- **Don't change the canvas shape.** 580 × 856 (0.68 : 1, portrait) — a
...
- [ ] **Hosed on Brady** — the false-front brick corner tavern — is in the top-right corner, roughly 158 × 88 px, and looks like what it is: not grander, not more civic, not a different kind of building.
...
- [ ] The canvas is 580 × 856 px.
- [ ] **No pavement, kerb or rounded corner along the bottom edge** — that edge is the cut through the block; the art runs straight off it.

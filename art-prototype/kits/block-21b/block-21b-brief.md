...
**You are painting HALF of one city block — the second half of block 21.**
...
- `block-21b-canvas.png` — **the stencil.** The white shape is the real block:
  the part of your painting we keep. Everything outside it is cut away. It is
  the exact size your painting must be.
- `block-21b-context.png` — **orientation only, never a thing to copy.** It shows
...
- `block-21b-layout.png` — **the site plan, and the most important attachment
...
- `block-21b-style-reference.png` — **a texture sample: HOW to paint, never WHAT
...
>
> Identical in size and shape to the attached `block-21b-canvas.png`. This is not a
...
- **One PNG, exactly 540 × 852 px, painted edge to edge.** That is 4× the block's
  final size on the board canvas (135 × 213 px at position x 293, y 1280 on the
...
- **The white shape in `block-21b-canvas.png` is what you are painting**: the real
...
**Paint in the manner of `block-21b-style-reference.png`** — its brushwork, not its
...
See `block-21b-context.png` — your block outlined in red dashes on the actual base
...
- **Cass Street Boys & Girls Club** (community_centre) — around (481, 187)
- **Three Holy Women Parish - Saint Rita Church** (place_of_worship) — around (462, 554)
- **Saint Rita Square** (social_facility) — around (273, 682)

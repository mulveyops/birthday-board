...
- **One transparent PNG, exactly 804 × 552 px.** That is 4× the block's
...
- **Paint only inside the white area of the stencil.** The attached
  `block-01-canvas.png` (same 804 × 552) is the paintable region, traced
...
See `block-01-context.png` — your block outlined in red dashes on the actual base
map (shown at 2×), so you can see the street geometry your edges meet.

## Hard constraints — must be exactly here, exactly this

1. **Fink’s** — anchor at canvas px **(1537, -35)** (the ground point of its entrance/front). Corner tavern — real Brady-area bar. Paint as a 2-story corner tavern with a modest sign band reading "Fink’s" (readable text allowed for this name).

2. **Scaffidi’s Hideout** — anchor at canvas px **(1529, 461)** (the ground point of its entrance/front). Corner tavern — real Brady-area bar. Paint as a 2-story corner tavern with a modest sign band reading "Scaffidi’s Hideout" (readable text allowed for this name).
